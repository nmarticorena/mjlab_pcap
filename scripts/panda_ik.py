...
Run with:
  MJLAB_WARP_QUIET=1 pixi run python scripts/demos/differential_ik_panda.py
"""
...
def main() -> None:
  ...
  robot.write_joint_position_to_sim(default_joint_pos, joint_ids=None)
  sim.forward()

  env = SimpleNamespace(num_envs=1, device=device, scene=sim_scene, sim=sim)
  ...
  zero_joint_vel = torch.zeros((1, len(PANDA_ARM_JOINTS)), device=device)
  physics_dt = sim.cfg.mujoco.timestep

  server = viser.ViserServer(label="Panda IK Control Demo")
  viewer_scene = ViserMujocoScene(server, sim.mj_model, num_envs=1)
  ...
  pos, quat = _get_frame_pose(ik_action)
  ...
  try:
    while True:
      ...
      for _ in range(int(iterations_slider.value)):
        ...
        q_target = robot.data.joint_pos[:, joint_ids] + dq
